"""Shared engine for the per-league players-registry builders.

Not runnable on its own. The league entry points (`registry/mlb/`,
`registry/nfl/`) call `main()` with their own defaults and `sources/`
directory; see the docstring in either of those files.
"""
...
def parse_args(default_registry_file):
...
def main(default_registry_file, source_dir):
    # `load_source_module` imports `sources.<name>` from the calling league's
    # directory. CPython already puts the entry point's directory on sys.path,
    # but make the dependency explicit rather than implicit.
    sys.path.insert(0, str(source_dir))

    args = parse_args(default_registry_file)
