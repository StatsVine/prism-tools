...
def load_fields(core_schema_path, source_schema_path):
    ...
    return list(core_schema.keys()) + [s['id_field'] for s in source_schema if s.get('active', True)]
...
def write_id_mappings(data, output_dir, id_fields):
    ...
    for id_field in id_fields:
        ...
        if not mapping:
            continue
        gzip_compress(write_json(mapping, Path(output_dir) / f"players.{id_field}.json", minified=False))
        gzip_compress(write_json(mapping, Path(output_dir) / f"players.{id_field}.min.json", minified=True))
