...
# Only the ids nflverse does not publish. DynastyProcess also carries gsis, pfr
# and espn, but validate_nflverse_ids.py already checks those -- adding them here
# would report the same mismatch twice, in two different issues.
MAPPINGS = {
    # DynastyProcess key : PRISM key
...
# DynastyProcess substitutes an Elias (ESB) id into the gsis_id column when the
# player has no real GSIS id, so pattern-check before using it as a join key.
GSIS_PATTERN = re.compile(r"^00-\d{7}$")


def clean(value: str) -> str:
    """DynastyProcess writes the R sentinel 'NA' rather than an empty field."""
    value = (value or "").strip()
    return "" if value == "NA" else value


def download_dynastyprocess_data(url: str = DYNASTYPROCESS_URL):
...
def build_index(dp_data: list[dict], key: str) -> dict:
    """
    Index DynastyProcess rows by one join key.
    ...
    for row in dp_data:
        ...
        if not value:
            continue
        if key == "gsis_id" and not GSIS_PATTERN.match(value):
            continue
...
def write_issues_txt(issues: list[dict], outfile_path: str = "issues.txt") -> None:
    ...
    header = [
        "## 📃 DynastyProcess differences found",
        "",
        f"_Generated {now} UTC_",
        "",
        "The following DynastyProcess mismatches found.",
        "",
        "| Prism ID | Name         | Key       | DynastyProcess Value | PRISM Value |",
        "|----------|--------------|-----------|----------------------|-------------|",
    ]
    ...
    for issue in issues:
        table_rows.append(
            ...
            f"| {issue['prism_key']} | {issue['dp_value']} "
...
def validate_csv(
    ...
    dp_data = download_dynastyprocess_data()
    dp_by_key = {key: build_index(dp_data, key) for key in MATCH_KEYS}
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for idx, row in enumerate(reader, start=1):
            ...
            for key in MATCH_KEYS:
                ...
                if our_id:
                    found = dp_by_key[key].get(our_id, None)
                if found:
                    break
            if not found:
                # not in DynastyProcess, skip
                continue
            else:
                matches += 1

            # Check our ID mappings against DynastyProcess's items
            for dp_key, our_key in MAPPINGS.items():
                is_ignore_key = False
                if ignores.get(prism_id, None):
                    if our_key in ignores[prism_id] or ignores[prism_id] == our_key:
                        is_ignore_key = True
                dp_val = clean(found.get(dp_key, ""))
                our_val = row.get(our_key, None)

                if not our_val and dp_val:
                    # present in DynastyProcess, not in PRISM
                    print(
                        f"Row {idx}, {prism_id}: Missing {our_key}, "
                        f"DynastyProcess has {dp_val}. Ignoring: {is_ignore_key}"
                    )
                ...
                elif dp_val and dp_val != our_val:
                    # Mismatch
                    print(
                        f"Row {idx}, {prism_id}: "
                        f"Diff {our_key}. DynastyProcess: {dp_val}, Prism: {our_val}. "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Detect mismatches between Prism IDs and DynastyProcess"
    )
